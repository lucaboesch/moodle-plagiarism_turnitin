...
class plagiarism_plugin_turnitin extends plagiarism_plugin {
    ...
    /**
     * Render the EULA form for the submission page.
     *
     * @param object $cm The course module.
     * @return string
    ...
    public function get_links($linkarray) {
        ...
        if (
            (!empty($linkarray["file"]) || !empty($linkarray["content"])) && !empty($linkarray["cmid"])
        ) {
            $this->load_page_components();
                    ...
                    if (
                        ...
                        if ($istutor || $linkarray["userid"] == $USER->id) {
                            $output .= html_writer::tag(
                                ...
                                $OUTPUT->pix_icon(
                                    'turnitin-icon',
                                    get_string('turnitinid', 'plagiarism_turnitin') . ': ' .
                        ...
                        if (
                            ...
                            if ($plagiarismfile->statuscode == "pending") {
                                ...
                                $orscorehtml = html_writer::tag(
                                    ...
                                    [
                                        'title' => $titlescore,
                                        'class' => 'tii_tooltip origreport_score score_colour ' . $class,
                                    ]
                                );
                            }
                        ...
                        if (!$istutor) {
                            ...
                            $output .= html_writer::tag(
                                ...
                                ['class' => 'clear plagiarism_turnitin_resubmit_link', 'id' => 'pp_resubmit_' . $plagiarismfile->id]
                            );
    ...
    public function cron_update_scores() {
        ...
        foreach ($submissions as $tiisubmission) {
            ...
            if (
                $tiisubmission->duedate_report_refresh != 1 && $now >= $dtdue && $now < strtotime('+1 day', $dtdue)
            ) {
        ...
        switch ($submissiontype) {
            ...
            case 'text_content':
                ...
                if ($previoussubmission) {
                    ...
                    if (
                        in_array($previoussubmission->statuscode, ["success", "error"])
    ...
    public function event_handler($eventdata) {
        ...
        */
        ...
        if (
            in_array(
                $eventdata['eventtype'],
                ["content_uploaded", "assessable_submitted"]
    ...
    public function clean_old_turnitin_submissions($cm, $userid, $itemid, $submissiontype, $identifier) {
        ...
        if ($submissiontype == 'file') {
            ...
            if (
                $moodlefiles = $DB->get_records_select(
                    'files',
                    " component = ? AND userid = ? AND source IS NOT null AND " .
                    $itemidsinsql,
                    $params,
                    'id DESC',
                    'pathnamehash'
...
function plagiarism_turnitin_coursemodule_standard_elements($formwrapper, $mform) {
    ...
    $pluginturnitin->add_settings_form_to_activity_page(
        $mform,
        $context,
        isset($formwrapper->get_current()->modulename) ? 'mod_' .
            $formwrapper->get_current()->modulename : ''
    );
}
